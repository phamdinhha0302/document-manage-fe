<template>
  <div
    class="folder-explorer h-full"
    @contextmenu="(e: MouseEvent) => handleRootContextMenu(e)"
  ...
  >
    ...
    <div style="margin-bottom: 16px; display: flex; gap: 8px; flex-wrap: wrap; align-items: center; justify-content: space-between">
      ...
      <div style="display: flex; gap: 8px">
        <Button :type="layoutType === 'grid' ? 'primary' : 'default'" @click="layoutType = 'grid'" title="Grid View">
          ⊞ Grid
        </Button>
        <Button :type="layoutType === 'list' ? 'primary' : 'default'" @click="layoutType = 'list'" title="List View">
          ☰ List
        </Button>
      </div>
    </div>
        ...
        <Col :xs="24">
          <div v-if="folders.length > 0 || isLoading">
            ...
            <Row v-if="layoutType === 'grid'" :gutter="[12, 12]">
              <Col v-if="isLoading && folders.length === 0" v-for="i in 4" :key="`skeleton-${i}`" :xs="12" :sm="8" :md="6" :lg="4" :xl="3">
                <Card style="height: 100%"><Skeleton active /></Card>
              </Col>
              <Col v-for="folder in folders" :key="folder._id" :xs="12" :sm="8" :md="6" :lg="4" :xl="3">
                ...
                >
                  ...
                  >
                    <template #cover>
                      <div style="padding: 15px 0; background: #fafafa; font-size: 32px; display: flex; align-items: center; justify-content: center; height: 80px">
                        <FolderOutlined />
                      </div>
                    </template>
                    <Card.Meta :title="folder.name" :description="folder.description || 'Untitled'" style="font-size: 13px" />
            ...
            <div v-else style="display: flex; flex-direction: column; gap: 8px">
              ...
              <div
                ...
                @click="navigateToFolder(folder._id)"
                @contextmenu.prevent.stop="showFolderContextMenu($event, folder)"
                :data-folder-list-id="folder._id"
                class="list-item"
                :style="{
              ...
              >
                ...
                <div style="flex: 1; min-width: 0">
                  ...
                  <div style="font-size: 12px; color: #999">{{ folder.description || 'Untitled' }} • {{ formatDate(folder.createdAt) }}</div>
                </div>
              </div>
            </div>
          </div>
      ...
      <Row :gutter="[16, 16]" style="margin-top: 24px" class="h-full">
        <Col :xs="24">
          <div v-if="documents.length > 0 || isLoading">
            <h3>Documents</h3>
            <Row v-if="layoutType === 'grid'" :gutter="[12, 12]">
              <Col v-if="isLoading && documents.length === 0" v-for="i in 4" :key="`doc-skeleton-${i}`" :xs="12" :sm="8" :md="6" :lg="4" :xl="3">
                <Card style="height: 100%"><Skeleton active /></Card>
              </Col>
              <Col v-for="doc in documents" :key="doc._id" :xs="12" :sm="8" :md="6" :lg="4" :xl="3">
                ...
                >
                  <template #cover>
                    <div style="padding: 15px 0; background: #f5f5f5; font-size: 32px; display: flex; align-items: center; justify-content: center; height: 80px">
                      <component :is="getFileIconComponent(doc.fileType)" />
                    </div>
                  </template>
                  <Card.Meta :title="doc.title" :description="doc.description || 'No desc'" style="font-size: 13px" />
                  <div style="margin-top: 6px">
                    <Tag :color="getCategoryColor(doc.category?.name)" style="font-size: 11px">{{ doc.category?.name }}</Tag>
                  </div>
            ...
            <div v-else style="display: flex; flex-direction: column; gap: 8px">
              <div v-if="isLoading && documents.length === 0" v-for="i in 3" :key="`doc-list-skeleton-${i}`" class="list-skeleton">
                <Skeleton active :paragraph="{ rows: 1 }" />
              </div>
              <div
                ...
                @click="viewDocument(doc._id)"
                @contextmenu.prevent.stop="showDocumentContextMenu($event, doc)"
                :data-doc-list-id="doc._id"
                class="list-item"
              >
                <div style="font-size: 24px"><component :is="getFileIconComponent(doc.fileType)" /></div>
                <div style="flex: 1; min-width: 0">
                  <div style="font-weight: 500; font-size: 14px; color: #000">{{ doc.title }}</div>
                  <div style="font-size: 12px; color: #999">
                    <Tag :color="getCategoryColor(doc.category?.name)" style="font-size: 10px; margin-right: 8px">{{ doc.category?.name }}</Tag>
                    {{ doc.description || 'No description' }} • {{ formatDate(doc.createdAt) }}
                  </div>
                </div>
              </div>
            </div>
    ...
    <Modal v-model:open="showCreateFolderModal" title="Create New Folder" @ok="handleCreateFolder">
      <Form :model="folderForm">
        <FormItem label="Folder Name" required><Input v-model:value="folderForm.name" /></FormItem>
        <FormItem label="Description"><Input v-model:value="folderForm.description" type="textarea" /></FormItem>
      </Form>
    </Modal>

    <Modal v-model:open="showEditFolderModal" title="Edit Folder" @ok="handleEditFolder">
      <Form :model="editingFolder">
        <FormItem label="Folder Name" required><Input v-model:value="editingFolder.name" /></FormItem>
        <FormItem label="Description"><Input v-model:value="editingFolder.description" type="textarea" /></FormItem>
      </Form>
    </Modal>

    <Modal v-model:open="showUploadModal" title="Upload Document" width="700px" @ok="handleUploadDocument" :confirm-loading="isUploading">
      <Form :model="uploadForm" layout="vertical">
        <FormItem label="Document Title" required>
        ...
        <FormItem label="Category" required>
          <div style="display: flex; gap: 8px">
            <Select v-model:value="uploadForm.categoryId" placeholder="Select a category" style="flex: 1">
              <SelectOption v-for="cat in categories" :key="cat._id" :value="cat._id">{{ cat.name }}</SelectOption>
            </Select>
            <Button @click="showCreateCategoryModal = true" type="dashed">+ New</Button>
          </div>
        </FormItem>
        <FormItem label="Tags">
          <div style="display: flex; gap: 8px">
            <Select v-model:value="uploadForm.tagIds" mode="multiple" placeholder="Select tags" style="flex: 1">
              <SelectOption v-for="tag in tags" :key="tag._id" :value="tag._id">{{ tag.name }}</SelectOption>
            </Select>
            <Button @click="showCreateTagModal = true" type="dashed">+ New</Button>
          </div>
        </FormItem>
        ...
        <div v-if="(uploadForm as any).isFolderUpload" class="folder-upload-info">
          ...
          <div>
            <strong>Folder Upload Mode</strong>
            <p style="margin: 0; font-size: 13px; color: #666;">
              System will recreate structure and upload valid files.
            </p>
          </div>
        </div>
        <FormItem v-else label="File" required>
    ...
    <Modal v-model:open="showCreateCategoryModal" title="Create New Category" @ok="handleCreateCategory">
      <Form :model="newCategoryForm">
        <FormItem label="Name" required><Input v-model:value="newCategoryForm.name" /></FormItem>
        <FormItem label="Description"><Input v-model:value="newCategoryForm.description" type="textarea" /></FormItem>
      </Form>
    </Modal>

    <Modal v-model:open="showCreateTagModal" title="Create New Tag" @ok="handleCreateTag">
      <Form :model="newTagForm">
        <FormItem label="Name" required><Input v-model:value="newTagForm.name" /></FormItem>
        <FormItem label="Description"><Input v-model:value="newTagForm.description" type="textarea" /></FormItem>
      </Form>
    </Modal>
...
import { message, Modal, Button, Card, Row, Col, Input, Form, FormItem, Spin, Breadcrumb, BreadcrumbItem, Tag, Dropdown, Menu, MenuItem, MenuDivider, Select, SelectOption, Upload, Skeleton } from 'ant-design-vue'
import axios from '@/api/axios.client'
import {
  FolderOutlined, FolderOpenOutlined, HomeOutlined, UploadOutlined, ReloadOutlined,
  FilePdfOutlined, FileImageOutlined, FileTextOutlined, FileOutlined
} from '@ant-design/icons-vue'
import { useFolders, useDocumentWithFolder } from '@/composables/useFolderComposable'
...
.drag-overlay {
    position: fixed; inset: 0; background: rgba(24, 144, 255, 0.1);
...
.drag-content {
    ...
    text-align: center; border: 2px dashed #1890ff; box-shadow: 0 4px 16px rgba(0,0,0,0.1);
}
.drag-icon { font-size: 48px; margin-bottom: 16px; }

/* List Styles */
.list-item {
    padding: 12px; border: 1px solid #d9d9d9; border-radius: 4px;
    display: flex; align-items: center; gap: 12px;
    background: #fff; cursor: pointer; transition: all 0.2s;
}
.list-item:hover { border-color: #1890ff; background: #f0f7ff; }
.list-skeleton { padding: 12px; border: 1px solid #d9d9d9; background: #fff; border-radius: 4px; }

.empty-state {
    position: absolute; inset: 0; min-height: 400px;
...
.folder-upload-info {
    margin-bottom: 24px; padding: 12px; background: #e6f7ff;
    border: 1px dashed #1890ff; border-radius: 4px;
    display: flex; align-items: start; gap: 10px;
}

/* Context Menu Trigger - Hidden but positioned */
.context-menu-trigger { position: fixed; width: 0; height: 0; }
</style>